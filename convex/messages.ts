...
const populateThread = async (ctx: QueryCtx, messagesId: Id<"messages">) => {
  const messages = await ctx.db
    .query("messages")
    .withIndex("by_parent_message_id", (q) => q.eq("parentMessageId", messagesId))
    .collect();
...
const populateReactions = (ctx: QueryCtx, messagesId: Id<"messages">) => {
  return ctx.db
    .query("reactions")
    .withIndex("by_messages_id", (q) => q.eq("messagesId", messagesId))
    .collect();
}
...
export const geyById = query({
  ...
  handler: async (ctx, args) => {
    ...
    const currentMember = await getMember(ctx, message.workspacesId, userId);
    ...
    const dedupedReactions = reactionsWithCounts.reduce(
      (acc, reaction) => {
        ...
        if (existingReaction) {
          existingReaction.memberIds = Array.from(new Set([...existingReaction.memberIds, reaction.membersId]));
        } else {
          acc.push({...reaction, memberIds: [reaction.membersId]});
        }
    ...
    const reactionsWithoutMemberIdProperty = dedupedReactions.map(({ membersId, ...rest }) => rest);
...
export const get = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!args.conversationsId && !args.channelsId && args.parentMessageId) {
    ...
    const results = await ctx.db
      .query("messages")
      .withIndex("by_channe_id_parent_message_id_conversation_id", (q) => 
        q
          .eq("channelsId", args.channelsId)
