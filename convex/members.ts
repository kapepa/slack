...
export const get = query({
  ...
  handler: async (ctx, args) => {
    ...
    const data = await ctx.db
      .query("members")
      .withIndex("by_workspace_id", (q) => q.eq("workspacesId", args.workspacesId))
      .collect();
...
export const remove = mutation({
  ...
  async handler(ctx, args) {
    ...
    if (currentMember.role !== "admin") throw new Error("Admin cannot be removed");
    if (currentMember._id === args.id && currentMember.role !== "admin") throw new Error("Cannot remove self of an admin");
    ...
    const [messages, reactions, conversations] = await Promise.all([
      ...
      ctx.db
        .query("reactions")
        .withIndex("by_members_id", (q) => q.eq("membersId", member._id))
        .collect(),
      ctx.db
        .query("conversations")
        .filter((q) => q.or(
